...
def visualize_agent_learning_curve(dataset_name, total_time_budget, df, alc):
    ...
    plt.step(df["total_time_spent"], df["score"], where="post", label=dataset_name)
    plt.scatter(df["total_time_spent"], df["score"], s=8, marker="D")
    L = plt.legend()
    L.get_texts()[0].set_text("ALC = " + str(np.around(alc, decimals=2)))
    plt.ylim(0, 1.05)
    plt.ylabel("test score")
    plt.fill_between(
        df["total_time_spent"], df["score"], step="post", alpha=0.4, color="dodgerblue"
    )
    plt.grid()
    ...
    if normalize_t:
        ...
        plt.text(1.0, df.iloc[-1]["score"], str(df.iloc[-1]["score"]), fontdict=None)
    else:
        plt.xlim(0, total_time_budget)
        plt.xlabel("time (s)")
        plt.text(
            df["total_time_spent"].max(),
            df.iloc[-1]["score"],
            str(df.iloc[-1]["score"]),
            fontdict=None,
        )
    ...
    plt.savefig(
        output_visualizations_dir + dataset_name + ".png",
        dpi=120,
        format="png",
        bbox_inches="tight",
    )
...
def write_scores_html(
    output_dir, output_visualizations_dir, auto_refresh=True, append=False
):
    ...
    try:  # Try to sort by numerical values
        image_paths = sorted(
            image_paths, key=lambda x: int(x.split("/")[-1].split(".")[0])
        )
    except:
...
def reveal_score_on_the_test_set(env, dataset_name, df):
    ...
    for i, row in df.iterrows():
        ...
        if algo_time_spent == 0:
            ...
            algo_index = int(row["algo_index"])
            # === Get the learning curve of the algorithm on the dataset at hand
            lc = env.test_learning_curves[dataset_name][list_algorithms[algo_index]]
...
def reveal_score_on_the_validation_set(env, dataset_name, df):
    ...
    for i, row in df.iterrows():
        ...
        if algo_time_spent == 0:
            ...
            algo_index = int(row["algo_index"])
            # === Get the learning curve of the algorithm on the dataset at hand
            lc = env.validation_learning_curves[dataset_name][
                list_algorithms[algo_index]
            ]
            index = list(lc.timestamps).index(algo_time_spent)
            score = lc.scores[index]
...
def compute_ALC(df, total_time_budget):
    ...
    >>> df
             algo_index  algo_time_spent  total_time_spent  score
    0            9            15.28          0.253961        0.39
    1            9            34.50          0.322162        0.44
    2            9            34.50          0.455340        0.44
    ...
    26           9           151.73          1.000000        0.49
    ...
    for i in range(len(df)):
        if i == 0:
            ...
            if normalize_t:
                alc += (df.iloc[i]["score"] - df.iloc[i - 1]["score"]) * (
                    1 - df.iloc[i]["total_time_spent"]
                )
            else:
                alc += (df.iloc[i]["score"] - df.iloc[i - 1]["score"]) * (
...
if __name__ == "__main__":
    ...
    if (
        ...
        output_from_ingestion_program_dir = (
            output_dir  # Output from the ingestion_program
        )
    else:
        ...
        algorithms_meta_features_dir = os.path.join(
            input_dir, "ref/algorithms_meta_features"
        )
        output_from_ingestion_program_dir = os.path.join(
            input_dir, "res"
        )  # Output from the ingestion_program
    ...
    vprint(
        verbose,
        "Using output_from_ingestion_program_dir: " + output_from_ingestion_program_dir,
    )
    ...
    for dataset_name in list_datasets:
        ...
        output_file = os.path.join(
            output_from_ingestion_program_dir, dataset_name + ".csv"
        )
        df = pd.read_csv(
            output_file,
            names=["algo_index", "algo_time_spent", "total_time_spent", "score"],
        )
