...
def visualize_agent_learning_curve(dataset_name, total_time_budget, df, alc):
    ...
    plt.fill_between(df['total_time_spent'], df['score'], step="post", alpha=0.4, color='dodgerblue')
    plt.grid()
    ...
    if normalize_t:
        ...
        plt.xlabel('time (s)')
        plt.text(df['total_time_spent'].max(), df.iloc[-1]['score'], str(df.iloc[-1]['score']), fontdict=None)
...
def reveal_score_on_the_validation_set(env, dataset_name, df):
    ...
    for i, row in df.iterrows():
        ...
        if algo_time_spent==0:
            ...
            algo_index = int(row['algo_index'])
            #=== Get the learning curve of the algorithm on the dataset at hand
            lc = env.validation_learning_curves[dataset_name][list_algorithms[algo_index]]
            index = list(lc.timestamps).index(algo_time_spent)
            score = lc.scores[index]
...
def compute_ALC(df, total_time_budget):
    ...
    for i in range(len(df)):
        if i==0:
            if normalize_t:
                alc += (df.iloc[i]['score']-0.0) * (1-df.iloc[i]['total_time_spent'])
            else:
                alc += (df.iloc[i]['score']-0.0) * (total_time_budget-df.iloc[i]['total_time_spent'])
        elif i>0:
            if normalize_t:
                alc += (df.iloc[i]['score']-df.iloc[i-1]['score']) * (1-df.iloc[i]['total_time_spent'])
            else:
                alc += (df.iloc[i]['score']-df.iloc[i-1]['score']) * (total_time_budget-df.iloc[i]['total_time_spent'])
